...
const ArtistasSidebar = ({ onAlbumSelect, artistas = [] }) => {
  ...
  // Si artistas está vacío, muestra al menos el artista por defecto
  const displayList =
    artistas.length > 0
      ? artistas
      : [
    ...
    >
      <div className="d-flex flex-column gap-3">
        ...
        {displayList.map((artista, index) => (
          ...
          >
            <img
              src={artista.image || artista.album?.image}
              className="rounded-circle mb-2"
              style={{ width: "70px", height: "70px", objectFit: "cover" }}
              alt={artista.name}
            />
            <span className="text-white small text-center">{artista.name}</span>
          </div>
        ))}
      </div>
    </div>
